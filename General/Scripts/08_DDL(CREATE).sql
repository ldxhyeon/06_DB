...
/* 자료형
    ...
    ** CHAR / VARCHAR2 주의사항 **
    - 오라클은 기본적으로 UTF-8 문자 인코딩을 사용하는데
    	...
    	NVARCHAR(크기) : "가변" 길이 문자형 (4000BYTE)
    	- BYTE 크기 단위가 아닌 글자 수 (문자 개수)로 크기를 지정하는 타입
    	
    	EX) VARCHAR2(3) : 가변길이 문자형 크기 3BYTE
    			NVARCHAR(3) : 가변길이 문자형 문자 3개까지 저장 가능
        
    DATE : 날짜 타입
    TIMESTAMP : 날짜 타입(MS + 단위 UTC) 
 ...
 *  비밀번호 : MEBER_PW /        VARCHAR2(20BYTE)
 * 	이름 :  MEMBERNAME /        VARCHAR2(15BYTE)
 *  주민등록 번호 : MEMBER_SSN /   CHAR(14BYTE) (- 포함)
...
INSERT INTO "MEMBER"
VALUES('MEM05', 'ASDFZXCV2323', '박철수232', '970303-1234567', DEFAULT);
...
-- NOT NULL 제약을 걸어서 NULL 값 포함되지 않음.
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KH_LDH.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_LDH"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-01400: NULL을 ("KH_LDH"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
/* 참조를 당하고 있는 값(GRADE_CODE) 10 삭제 해보기 */
DELETE
FROM
	USER_GRADE
WHERE
	GRADE_CODE = 10; 
-- ORA-02292: 무결성 제약조건(KH_LDH.GRADE_CODE_FK1)이 위배되었습니다
-- - 자식 레코드가 발견되었습니다
-- 자식 테이블인 USER_USDE_FK 테이블에서
...
CREATE TABLE USER_TEST (
	USER_NO NUMBER, 
	USER_ID VARCHAR2(20),
	USER_PWD VARCHAR2(15) CONSTRAINT NN_USER_PWD NOT NULL,
  ...
  -- 테이블 레벨
	...
	CONSTRAINT UK_USER_ID   UNIQUE(USER_ID), -- PK
