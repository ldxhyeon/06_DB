...
FROM
	EMPLOYEE
GROUP BY
	DEPT_CODE;  -- 같은 DEPT_CODE 끼리 묶음(그룹)
...
ORDER BY
	DEPT_CODE ASC;
--> EMPLOYEE 테이블에서 DEPT_CODE 기준으로 만들어진 그릅 중
...
GROUP BY
	JOB_CODE
ORDER BY
	JOB_CODE ASC; -- 4행 조회(0명은 조회결과에 포함X)
