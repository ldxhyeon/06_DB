...
JOIN 
	JOB USING(JOB_CODE)
WHERE
	JOB_NAME = '대리' -- 332만, 300만
	
AND SALARY > ANY  (
							 SELECT
								SALARY -- 476만, 320만, 350만
							 FROM
								EMPLOYEE
							 JOIN 
								JOB USING(JOB_CODE)
...
WHERE 
	JOB_NAME = '과장'
	AND   
	SALARY > ALL ( -- 380, 348, 349, 255만 전체보다 큰 경우
						SELECT SALARY
						FROM EMPLOYEE
						JOIN JOB USING(JOB_CODE)
						WHERE JOB_NAME = '차장'
...
-- 상관 서브쿼리는
-- 메인쿼리 1행 해석 -> 서브쿼리 수행 -> 다시 메인쿼리 1행 해석 ->	서브쿼리 해석
...
WHERE
	...
	WHERE NVL("SUB".DEPT_CODE, '소속없음') = NVL("MAIN".DEPT_CODE, '소속없음')
						--> NULL은 비교가 안되기 떄문에 가능한 형태로 변환
	AND ENT_YN != 'Y' --> 퇴사자 제외
	
	--> 서브쿼리에서 조회된 결과를 다시 메인쿼리로 넘겨
	-- 메인쿼리 WHERE 조건을 충족하는지 확인
		);
...
-- 7. 인라인 뷰(INLINE-VIEW) --> SELECT문에 조회되는 가상 테이블
...
FROM
	...
	ORDER BY
		"급여 평균" DESC)
WHERE
	ROWNUM <= 3; -- 서브 쿼리 조회 결과에서 1,2,3행만 조회
...
-- 단, 값의 크기가 같다면  공동 순위 지정, 지정된 만큼 후 순위 건너뛰기
SELECT
...
-- 항상 범위에 1부터 연속적인 범위가 포함 되어야만 한다!
	--> ROWNUM은 RESULT SET 완성 후 적용되는 가삼 컬럼이라서
	-- 정해진 규직 (1부터 1씩 증가)의 만족하지 못하면 사용 불가 
