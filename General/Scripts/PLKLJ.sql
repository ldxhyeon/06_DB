...
WHERE
    PIECE_TYPE = 2
    AND PIECE_STATUS = 'S'
    AND TO_DATE(PA.START_DATE, 'YYYY-MM-DD HH24:MI:SS') >= TRUNC(SYSDATE) 
ORDER BY
    TO_DATE(PA.START_DATE, 'YYYY-MM-DD HH24:MI:SS') ASC
FETCH FIRST 3 ROWS ONLY;
...
WHERE
    PIECE_TYPE = 2
    AND PIECE_STATUS = 'S'
    AND TRUNC(PA.START_DATE) = TRUNC(SYSDATE) -- START_DATE가 오늘 날짜
ORDER BY
    PA.START_DATE ASC
FETCH FIRST 3 ROWS ONLY;
 ...
 SELECT
	        ...
	        P.PIECE_RENAME,
	        PA.START_DATE,
	        (SELECT COUNT(*) FROM "WISH" WHERE PIECE_NO = 114 AND MEMBER_NO = 1) AS LIKE_CHECK,
          A.ARTIST_NICKNAME
    	 FROM
            "PIECE" P
        JOIN
