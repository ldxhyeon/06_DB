...
/* 제목 검색인 경우 */
/*AND BOARD_TITLE LIKE '11' || '콩' || '%';*/

-- 내용 검새인 경우
/*AND BOARD_CONTENT LIKE '%' || '나' || '%'*/

-- 제목 또는 내용 검색
AND (BOARD_TITLE LIKE '%' || '콩' || '%'
...
COMMENT ON COLUMN "NOTFICATION"."NOTIFICATION_NO" IS '알림번호';

COMMENT ON COLUMN "NOTFICATION"."NOTIFICATION_CONTENT" IS '알림내용(HTML 포함)';

COMMENT ON COLUMN "NOTFICATION"."NOTIFICATION_CHECK" IS '읽음 여부(Y,N)';

COMMENT ON COLUMN "NOTFICATION"."NOTIFICATION_DATE" IS '알림 생성 시간';

COMMENT ON COLUMN "NOTFICATION"."NOTIFICATION_URL" IS '연결 페이지 주소';

COMMENT ON COLUMN "NOTFICATION"."SEND_MEMBER_NO" IS '알림 보낸 회원 번호';

COMMENT ON COLUMN "NOTFICATION"."RECEIVE_MEMBER_NO" IS '알림 받는 회원 번호';

ALTER TABLE "NOTFICATION" ADD CONSTRAINT "PK_NOTFICATION" PRIMARY KEY (
	"NOTIFICATION_NO"
);

ALTER TABLE "NOTFICATION" ADD CONSTRAINT "FK_MEMBER_TO_NOTFICATION_1" FOREIGN KEY (
	"SEND_MEMBER_NO"
)
REFERENCES "MEMBER" (
	"MEMBER_NO"
...
ALTER TABLE "NOTFICATION" ADD CONSTRAINT "FK_MEMBER_TO_NOTFICATION_2" FOREIGN KEY (
	"RECEIVE_MEMBER_NO"
)
REFERENCES "MEMBER" (
	"MEMBER_NO"
...
-- * 검색 순서 -1 == 뒤에서부터 검색
SELECT
	SUBSTR(PROFILE_IMG, INSTR(PROFILE_IMG ,'/', -1) +1)
	AS "FILE_NAME"
FROM
	"MEMBER"
WHERE
