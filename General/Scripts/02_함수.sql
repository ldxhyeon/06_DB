...
FROM 
	"NATIONAL"; -- "테이블명" == 이름이 "테이블명"과 같은 테이블
 ...
 * 		MONTH_BETEWEEN을 이용하면 이런 부분까지 모두
 * 		자동으로 적용되어 계산된다!
 * */
...
FROM
	EMPLOYEE
WHERE
	/*HIRE_DATE BETWEEN '2010-01-01' AND '2019-12-31'*/
	EXTRACT(YEAR FROM HIRE_DATE) BETWEEN 2010 AND 2019
ORDER BY
	HIRE_DATE DESC; -- 10행 조회
...
-- 지정된 칸 내부에서 문자열
SELECT
	1234, TO_CHAR(1234, '999999999')
FROM
	DUAL;

SELECT
	1234, TO_CHAR(1234, '000000000')
FROM
	DUAL;

/* 숫자 -> 문자열 반환 시 발생할 수 있는 문제 상황 */
--> 지정된 포맷의 칸 수가 변환하려는 숫자 자릴수 보다 적은 경우
-- 모든 숫자가 #으로 변환되서 출력
SELECT
	1234, TO_CHAR(1234, '999')
FROM
	DUAL;
...
-- 50 미만 확인
SELECT
	TO_DATE('50-12-25', 'YY-MM-DD'), -- 2049
	TO_DATE('50-12-25', 'RR-MM-DD') -- 2049
FROM 
	DUAL;
