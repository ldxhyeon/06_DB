...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
		--> DEPT_CODE와 DEPT_ID 값이 같은 행을 한 줄로 합쳐라
...
FROM
	DEPARTMENT
JOIN 
	LOCATION ON (DEPT_ID = LOCATION_ID);


--> INNER JOIN의 기본 값 (생략가능)
...
FROM
	EMPLOYEE E
JOIN
	SAL_GREADE S ON(E.SALARY BETWEEN S.min_SAL AND S.MAX SAL);
---------------------------------------------------------------------------------------------------------------
...
FROM
	EMPLOYEE A
JOIN 
	EMPLOYEE B ON(A.DEPT_CODE = B.DEPT_CODE)
WHERE
